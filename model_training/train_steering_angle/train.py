...
class Trainer:
    ...
    def _log_progress(self, epoch, step, batch_size):
        ...
        feed = {model.x: xs, model.y_: ys, model.keep_prob: 1.0}

        loss_value = self.loss.eval(feed_dict=feed)
        print(f"Epoch: {epoch+1}, Step: {step}, Loss: {loss_value:.5f}")

        summary = self.merged_summary_op.eval(feed_dict=feed)
        global_step = epoch * int(driving_data.num_images / batch_size) + step
        self.logger.log_summary(summary, global_step)
